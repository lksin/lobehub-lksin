...
/**
 * Aggregate live metrics off the subagent's child messages. Persisted
 * `thread.metadata.total*` (written on finalize) takes precedence so the
 * historical view doesn't have to walk the message list, but if those
 * fields are missing — i.e. the subagent is still streaming or persistence
 * predates this field — we sum from `usage.totalTokens` per assistant
 * message and count `role==='tool'` messages so the chip updates live.
 *
 * `model` is read off the first assistant message that carries one, since
 * CC subagents normally pin a single model for the whole run.
 */
const useSubagentMetrics = (toolCallId: string | undefined): SubagentMetrics | null =>
  useChatStore((s) => {
    ...
    const messages = threadSelectors.getThreadChildMessages(thread.id)(s);
    ...
    if (persistedTokens !== undefined) totalTokens = persistedTokens;
